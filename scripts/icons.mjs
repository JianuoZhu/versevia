...
await writeFile('extension/icons/versevia.svg', svg);
for (const width of [16, 32, 48, 128]) {
  const png = new Resvg(svg, { fitTo: { mode: 'width', value: width } }).render().asPng();
  await writeFile(`extension/icons/versevia-${width}.png`, png);
}
console.log('Exported shared Versevia mark at 16, 32, 48 and 128 px.');
